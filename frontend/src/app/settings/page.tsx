              ...
              {(() => {
                ...
                const areas: {
                  ...
                  },
                  {
                    ...
                    gaps: 'The "enforced" label is a display tag only — no engine actually blocks non-conforming data. No email or Slack notification sent to approvers on submission. No audit trail UI showing who approved what and when (approval history is not persisted or displayed). No policy versioning or change history. No notifications when a policy is violated.',
                  },
                  {
                  ...
                  },
                  {
                    ...
                    icon: '🔐',
                    status: 'partial',
                    exists: 'Compliance page lists GDPR, HIPAA, and SOC 2 as framework cards with pass/fail counts and a percentage bar. Controls table fetches from /api/compliance/{framework_id}/controls — populating control code, name, description, status (passed/failed/not_assessed), rule types covered, rules mapped count, last assessed date, and evidence links. Filter buttons (all/passed/failed/not-assessed) are functional. "Initialize Frameworks" button seeds data via /api/compliance/seed. "Assess All Assets" triggers a run. "Export Evidence" calls /api/audit/evidence-report with configurable day range. Dashboard Compliance tile surfaces framework compliance counts with a green/amber/red status pill.',
                    gaps: 'Controls table is empty without manual seeding — no auto-mapping logic to populate controls from active rules. Compliance page KPI summary cards still show "—" when no aggregate data is returned. No data masking or anonymization capability. No right-to-erasure or data subject request workflow. No consent management. No data residency configuration. No query-time dynamic data masking for PII columns.',
                  },
                  {
