...
import { DEMO_GOVERNANCE_VIOLATIONS } from '@/lib/demoData'
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!res.ok) return NextResponse.json(DEMO_GOVERNANCE_VIOLATIONS)
    const data = await res.json()
    const arr = Array.isArray(data) ? data : []
    return NextResponse.json(arr.slice(0, limit))
  } catch { return NextResponse.json(DEMO_GOVERNANCE_VIOLATIONS) }
}
