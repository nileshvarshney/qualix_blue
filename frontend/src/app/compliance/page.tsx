    ...
    <div style={{ padding: '28px 36px', maxWidth: '1300px' }}>
      ...
      <div style={{ ...card, marginBottom: '20px' }}>
        ...
        {loading ? (
          ...
          <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8', borderRadius: '12px', border: '2px dashed #ebe8df' }}>No compliance frameworks configured</div>
        ) : (
            ...
            {frameworks.map(fw => {
              ...
              return (
                <div key={fw.id} onClick={() => setSelectedFw(isSelected ? null : fw.id)} style={{
                  border: `1px solid ${isSelected ? '#93c5fd' : '#ebe8df'}`, borderRadius: '10px',
                  padding: '14px 16px', cursor: 'pointer', background: isSelected ? '#f0f9ff' : '#fff',
                  ...
                  <div style={{ fontSize: '11.5px', color: '#475569' }}>
                    <span style={{ fontWeight: 600, color: '#16a34a' }}>{fw.controlsPassed}</span>
      ...
      <div style={card}>
        ...
        {!selectedFw ? (
          <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8', borderRadius: '12px', border: '2px dashed #ebe8df' }}>
            Select a framework above to view its controls
          </div>
        ) : controlsLoading ? (
          <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8' }}>Loading controls…</div>
              ...
              {filteredControls.map(c => {
                ...
                return (
                  <tr key={c.id} style={{ borderBottom: '1px solid #f3f1ea' }}>
                    ...
                    <td style={{ padding: '12px', fontWeight: 500, color: '#1a1a1a', maxWidth: '220px' }}>
                      <div>{c.name}</div>
                      {c.description && <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '2px' }}>{c.description.slice(0, 80)}{c.description.length > 80 ? '…' : ''}</div>}
                    </td>
                    <td style={{ padding: '12px', color: '#64748b', fontSize: '11px', fontFamily: 'monospace' }}>
                      {c.ruleTypes ? c.ruleTypes.split(',').map(rt => (
                        <span key={rt} style={{ display: 'inline-block', background: '#f1f5f9', borderRadius: '4px', padding: '1px 6px', marginRight: '4px', marginBottom: '2px' }}>{rt.trim()}</span>
                      )) : '—'}
                    </td>
                    ...
                    <td style={{ padding: '12px', color: '#475569', textAlign: 'center' }}>{c.rulesMapped || '—'}</td>
                    <td style={{ padding: '12px', color: '#94a3b8', whiteSpace: 'nowrap' }}>{c.lastAssessed ?? 'Never'}</td>
