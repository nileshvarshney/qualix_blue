# Grouped Execution Logs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Change the Execution Logs page so each row represents a job execution against a table/view (a batch of rule runs clustered within 5 minutes of each other), instead of one row per individual rule run.

**Architecture:** Extract `ExecLog` type, raw-data mapping, and a pure client-side grouping function into a new `frontend/src/lib/executionLogs.ts` module (following the existing `lib/schedules.ts` pattern). Rewrite `frontend/src/app/execution-logs/page.tsx` to consume `GroupedExecLog[]`, with a two-level expand (group → member rule → rule detail panel), updated stat cards, filters, and CSV export.

**Tech Stack:** Next.js (App Router), React (client components), TypeScript. No backend changes. No test runner exists for the frontend — verification is via `npx tsc --noEmit`, `npm run lint`, and a manual browser check.

---

### Task 1: Create `lib/executionLogs.ts` with types and grouping logic

**Files:**
- Create: `frontend/src/lib/executionLogs.ts`
...
Expected: No errors reported for `src/lib/executionLogs.ts` (errors in other files at this point, if any, are pre-existing — confirm by running the same command on `main` first if unsure).

- [ ] **Step 3: Commit**

```bash
git add frontend/src/lib/executionLogs.ts
git commit -m "feat(execution-logs): add ExecLog types and job-execution grouping"
```

---

### Task 2: Rewrite the Execution Logs page to use grouped rows

**Files:**
- Modify: `frontend/src/app/execution-logs/page.tsx` (full rewrite of the component body)
...
Expected: No new errors/warnings introduced by `src/app/execution-logs/page.tsx` or `src/lib/executionLogs.ts`.

- [ ] **Step 3: Commit**

```bash
git add frontend/src/app/execution-logs/page.tsx
git commit -m "feat(execution-logs): group log rows by table/view job execution"
```
...
Expected: Server starts on `http://localhost:3000` (or printed port) without compile errors.

- [ ] **Step 2: Open the Execution Logs page and verify grouping**

Navigate to `http://localhost:3000/execution-logs`. Verify:
- Each row represents a table/view (shows `Table/View · Connection` and a `N rules` badge), not a single rule.
- If the seeded/dev data contains multiple rule runs for the same table within 5 minutes of each other, they appear as one row with `checked`/`failed` summed and `score` averaged.
- Clicking a row expands to show its member rules (Rule · Type, Status, Score, Checked/Failed, Duration).
- Clicking a member rule expands the existing detail panel (Failure Reason, Root Cause, Impact/Recommendation, Query/Error Sample) below it.
- Stat cards (Total/Passed/Failed/Warnings/Avg Score) reflect group counts, and clicking a card filters groups by status.
- Search filters by table/view name or any member rule's name.
- "Export CSV" downloads one row per individual rule run (same columns as before).

- [ ] **Step 3: Stop the dev server**

Stop the process started in Step 1 (e.g. Ctrl-C).
